...
- 2022-03_instructgpt: the PPO-based RLHF pipeline that GRPO simplifies; GRPO keeps its clipping and KL-to-reference ideas.
- 2023-05_dpo: fellow "remove a model from the RLHF stack" method; the unified paradigm here places DPO as offline, rule-rewarded, pairwise GC.
- 2025-01_deepseek-r1: GRPO at scale with verifiable rewards; the direct descendant.
- 2024-12_deepseek-v3: the model family whose post-training inherits this pipeline.
- KB topics: topics/rl (RL for LLMs, RLVR, GRPO), topics/llm-training-and-post-training (alignment), topics/data-curation-and-datasets (CC mining pipeline).
