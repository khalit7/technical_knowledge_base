...
- 2024-02_deepseekmath-grpo: the underlying policy update; SA-MRPO changes only the advantage construction and keeps the clipped GRPO surrogate.
- 2025-01_deepseek-r1: the RLVR regime (rule-based verifiable rewards) this method assumes; R1-style recipes with format + correctness rewards are exactly the saturation scenario in Figure 1.
...
- KB topics: topics/rl (RL for LLMs, RLVR, GRPO), topics/llm-training-and-post-training (alignment, reward shaping).
