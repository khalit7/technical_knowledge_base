...
- DDPM (2020-06): supplies the entire diffusion objective and epsilon-prediction UNet; LDM changes only where the chain runs.
- CLIP (2021-02): this paper uses a BERT-style text transformer, but Stable Diffusion swapped in CLIP's text encoder as tau_theta.
- ViT (2020-10): the DiT successors replace LDM's UNet with a transformer while keeping its latent space.
- Attention Is All You Need (2017-06): cross-attention conditioning is the transformer's encoder-decoder attention grafted into a UNet.
- Topics: generative-and-multimodal (diffusion, VAE, text-to-image).
