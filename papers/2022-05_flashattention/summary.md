...
- [Attention Is All You Need (2017)](../2017-06_attention-is-all-you-need/): the O(N^2) attention this paper makes fast without changing its output.
- [vLLM / PagedAttention (2023)](../2023-09_vllm-pagedattention/): the complementary inference-side memory result; FlashAttention optimises HBM-SRAM traffic inside the kernel, PagedAttention optimises KV-cache allocation across requests. Serving stacks use both.
- [Megatron-LM (2019)](../2019-09_megatron-lm/): the training-systems baseline FlashAttention beats and was merged into.
- [Mamba (2023)](../2023-12_mamba/): Tri Dao's other line of attack on sequence length; its selective-scan kernel reuses the same IO-aware recipe (fuse, keep state in SRAM, recompute in backward).
- [DeepSeek-V3 (2024)](../2024-12_deepseek-v3/) and every modern LLM report: FlashAttention-class kernels are assumed infrastructure for the published long-context and MFU numbers.
