...
- Read alongside **StateM** and **Prime Agent** in this folder; the three together are the current state of the harness-scaling argument.
...
- **Prime Agent** (in this folder): the mirror image. Prime Agent gives an untrained model a rich harness and notes that models are not trained to operate it; JIT-Agent trains a model to build the harness but keeps the harness simple. Neither has yet done both.
- **StateM** (in this folder): the hand-built harness whose lift JIT-Agent is trying to automate; StateM's versioned YAML runbook is roughly a hand-tuned `(M, P, A, F)` instance.
- **Demystifying Agent Skills** (in this folder): the `F` module is the skills question, and the 10% misapplication rate found there is a plausible source of JIT-Agent's residual failures.
- **SA-MRPO** (in this folder): Evo-GDPO's separate normalisation of reward, latency, and cost is the same multi-objective RL problem, approached through frontier comparison rather than saturation-aware reweighting.
- Topic: agentic-harnesses (harness engineering); Topic: agentic-frameworks.
