...
- InstructGPT: the RLHF recipe CAI extends; CAI swaps the human preference labels for AI labels (harmlessness only) and keeps the PM + RL machinery.
- DPO: later removes the PM/RL machinery entirely; modern pipelines often combine CAI-style AI preference data with DPO-style direct optimization.
- DeepSeekMath / GRPO and DeepSeek-R1: the RL-for-LLMs line that CAI's RLAIF stage belongs to; R1 replaces preference rewards with verifiable rewards, the other route around human labels.
