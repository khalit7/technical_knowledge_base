...
- **Benchmarks**: Terminal-Bench 2.1 (primary), BusinessBench (task-family generalization); adjudication and submission-pipeline mechanics are a good case study for topics/benchmarks.
- **Agentic harnesses**: builds on and contrasts with Codex and Claude Code control fragments (plans, memory files, hooks); the stop-hook integration targets both. See topics/agentic-harnesses.
- **Orchestration frameworks**: StateFlow (state-driven execution) and LangGraph (durable graph runtimes) provide the capabilities StateM reorganizes around the agent instead of around a developer controller; relevant to topics/agentic-frameworks.
...
- **In this repo**: 2026-08_envharness and 2026-08_agent-skills sit in the same harness-over-model wave; the runbook-as-procedural-memory idea also connects to skills-as-files in agent harnesses.
