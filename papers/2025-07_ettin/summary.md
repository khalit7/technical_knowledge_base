...
- Related papers here: BERT (the encoder lineage Ettin modernizes), GPT-3 (the decoder shift it interrogates), OLMo 2 (supplies the DCLM/Dolma/Dolmino data sources and the open-artifact philosophy; Pythia inspired the checkpoint release), RoFormer/RoPE (context extension via RoPE theta scaling), Chinchilla (over-training far past compute-optimal is what made decoder adaptation popular)
