...
- Papers in this KB: 2017-06_attention-is-all-you-need (the layer structure being partitioned), 2018-10_bert (the architecture whose LN placement it fixes), 2019-10_zero (the contemporaneous memory-side alternative: ZeRO shards optimizer state under data parallelism instead of partitioning layers), 2020-05_gpt-3 (trained with model parallelism at the scale this line of work enabled), 2024-07_llama-3 (4D parallelism, TP included, in a modern frontier run), 2024-12_deepseek-v3 (notable for engineering around TP and dropping it in favour of EP/PP/ZeRO-1)
- Topics: llm-training-and-post-training (distributed training: the TP axis), inference-and-serving (TP as the default multi-GPU serving strategy), hardware (NVLink/NVSwitch bandwidth as the TP constraint)
