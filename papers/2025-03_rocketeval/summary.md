...
- Read it against the KB's [LLM-as-judge: design, biases, calibration, reliability](../../topics/evaluation-and-llm-judges/llm-as-judge.md) page, whose bias catalogue explains *why* the reformulation works.
...
- **LLM-as-judge** (evaluation-and-llm-judges): a concrete answer to the position, verbosity and self-preference bias catalogue, removing the channel rather than debiasing after the fact. It sits next to the juries/PoLL result: both say many cheap decisions beat one expensive one.
- **Production eval engineering** (evaluation-and-llm-judges): the cost profile is what makes a per-commit regression gate realistic; the instance-level number is why you gate on aggregates, not single cases.
- **Constitutional AI** (Papers): the same shape one level up, a written rubric standing in for human judgment at training time rather than eval time.
- **Demystifying Agent Skills** (Papers): a checklist here is a procedural anchor there. Both find that writing the procedure down explicitly is what transfers, and that the artifact beats the model's improvisation.
- **StateM** and **JIT-Agent** (Papers): the same "expensive artifact, cheap execution" economics in the harness layer, where an expensive run produces a runbook or a synthesised harness that a cheaper model then executes.
- **Speculative decoding** (inference-and-serving): the same economics inverted, cheap model proposes and expensive process verifies, rather than expensive model specifies and cheap model executes.
