...
- papers/2021-01_switch-transformer: the top-1 MoE predecessor; Mixtral's top-2, every-layer, no-fuss variant is what finally made the idea land in open weights.
- papers/2024-12_deepseek-v3: the next generation of open MoE; fine-grained experts, shared experts, and aux-loss-free balancing respond directly to the coarse-expert, no-domain-specialisation regime documented here.
- papers/2023-05_dpo: Mixtral Instruct was among the first flagship models aligned with SFT+DPO only.
- papers/2023-09_vllm-pagedattention: Mistral upstreamed Megablocks-based MoE kernels into vLLM to make the model servable at release.
- papers/2017-06_attention-is-all-you-need: the base decoder-only transformer whose FFN sub-block the MoE layer replaces.
