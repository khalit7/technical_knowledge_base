...
- Companion pages kept as-is and renamed in the file map: [python-staying-current.md](../topics/programming-languages/python-staying-current.md), [cpp-staying-current.md](../topics/programming-languages/cpp-staying-current.md).
...
- **Rust**: the shortest section, because cargo is uncontested. crates.io, `Cargo.toml` versus `Cargo.lock`, the 0.x semver rule where minor is the breaking position, multiple major versions coexisting in one binary, feature unification, workspaces, `cargo install`, docs.rs, and `cargo audit`/`deny`/`vendor` plus the build-script equivalent of npm `postinstall` risk.
