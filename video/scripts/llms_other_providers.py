...
VISUALS = {
    ...
    "phi": {"kind": "stat", "focus": "data quality per parameter",
            "big": "84.8%", "tone": "number",
    ...
    "cohere": {"kind": "bars", "focus": "auditability, on-prem",
    ...
    "close": {"kind": "claim",
              ...
              "note": "Cohere judged by a rival's model, Tencent's own 31.8%: "
                      "vendor claims until somebody reruns them"},
}
...
SCRIPT["ident"] = [
    (A, "This is the long tail of language model providers, as the knowledge "
        ...
        "DeepSeek."),
    (A, "There is one way to read this page that works. None of these labs is "
        "trying to top a leaderboard, and read as failed frontier labs every "
        "one of them looks like a disappointment."),
]

# 1. the inventory, organised by constraint rather than by country
SCRIPT["map"] = [
    (A, "So group them by what each is actually optimising. Data quality per "
        "parameter, which is Microsoft's Phi line and the small models around "
        "it. Distribution and billing, which is Amazon: Nova, Bedrock, and "
        "the Trainium accelerator."),
    (A, "Auditability and on-premises deployment, which is Cohere and I B M's "
        "Granite. And memory per token while serving, which is N V I D I A's "
...
SCRIPT["question"] = [
    ...
    (A, "You do not. You ask a different question of each. Not how good is "
        "it, but what does it make cheap, or possible, that a frontier A P I "
        "does not?"),
    (A, "Three of them carry a real idea, so they get a beat each. Then the "
        "fifth group, quickly."),
]
...
SCRIPT["phi"] = [
    (A, "Microsoft's Phi is one hypothesis, tested hard. Most of a "
        ...
        "raw web."),
    (A, "Textbook quality is the label. Web filtered by a classifier scoring "
        "documents for reasoning value rather than fluency, and a corpus "
        "mostly synthetic: worked solutions from a larger model, built so "
        "each one demonstrates a step rather than states a fact. Phi four, "
        "fourteen billion parameters, eighty four point eight percent on M M "
        "L U."),
    (B, "If that works at fourteen billion, why is everybody not doing it?"),
    (A, "Because the cost is in the same sentence as the result. A model whose "
        "training data is synthesised from benchmark-shaped tasks is "
        "strongest on benchmark-shaped tasks, and weaker on long-tail world "
        "knowledge and messy real input."),
]

# 4. memory per token, which is a serving argument and not a quality one
SCRIPT["memory"] = [
    (A, "Second constraint, and the most operationally useful. N V I D I A "
        "gives models away as demand generation for graphics cards, and the "
        "Nemotron Nano models are the interesting part, because they are not "
        "transformers. They are Mamba two hybrids."),
    (A, "A state space model carries a fixed-size hidden state through the "
        "sequence, so memory per token stays constant however long the "
        "context is. Mamba two makes that recurrence selective, so the block "
        "chooses what to keep, and shapes it as a matrix multiplication so it "
        "trains on the tensor cores."),
    (A, "The number is on the right. Key-value cache per token falls from "
        "hundreds of kibibytes to a few, because only the attention layers "
        "cache at all. Where memory rather than arithmetic is the limit, that "
        "moves the economics far more than a couple of benchmark points."),
]
...
SCRIPT["cohere"] = [
    (A, "Third, Cohere, who sell to enterprises that will not send data to a "
        ...
        "becomes visible to a reviewer. Command A is dense rather than "
        "mixture of experts on purpose, because a dense hundred and eleven "
        "billion fits two graphics cards predictably."),
    (A, "Then this. North Small Translate is two hundred and eighteen billion "
        "parameters built to do one thing, and it scores eighty three point "
        "six on W M T twenty six, against DeepL at eighty one point three "
        "seven and Google Translate at sixty eight point two."),
    (B, "Who ran that evaluation?"),
    (A, "Cohere did, with a rival's model as judge. Hold it loosely. What "
        "survives the caveat is the shape: a single-task specialist at that "
        "scale is still viable against generalists."),
]

# 6. the fifth group, named once each and not explained
SCRIPT["sweep"] = [
    (A, "Which leaves the fifth group, and each is here for one reason. Prime "
        "Intellect train one model across clusters in different places joined "
        "by ordinary internet links, which says frontier training need not be "
        "centralised."),
    (A, "The Institute of Foundation Models is the second lab, after Ai two, "
        ...
        "streamed a post-training run's metrics live and released no model, "
        "so the dashboard is the artifact. And Prism rebuilt a twenty seven "
        "billion parameter model at one point seven six bits per weight."),
]

# 7. the take
SCRIPT["close"] = [
    (A, "One last thing, and it applies to everything you just heard. "
        "Cohere's translation scores were judged by Cohere. Tencent's claim "
        "that its own model bought it a thirty one point eight percent "
        "throughput gain is Tencent's."),
    (A, "That is not a reason to ignore them. It is why the useful question "
        "about a long-tail lab is never how good it is. It is what it makes "
        "cheap, or possible, and that part you can check yourself."),
]
