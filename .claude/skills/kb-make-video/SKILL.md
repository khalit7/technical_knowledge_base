...
*This copy is the page as Notion last edited it, 2026-09-22 19:24:00 UTC. A procedure*
...
**A single rich-text run caps at 2,000 characters.** A long paragraph has to be split across several runs inside the one block. Exceeding it fails loudly with a validation error rather than truncating quietly, at least through `POST /pages`, so the bad case is the one you cannot miss. The obvious MCP call gets the position wrong silently, as the publishing section explains.
