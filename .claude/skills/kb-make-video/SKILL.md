...
*This copy is the page as Notion last edited it, 2026-09-22 13:43:00 UTC. A procedure*
...
10b. **Set every ****`reserve`**** from the rendered durations, then render the animation again.** The right value is arithmetic and it is unknowable until the voice exists: `spread` divides `beat_length - reserve` evenly, so reveal k of n lands at `k/n x (beat_length - reserve)`. If the narration names item k at second t, you need `reserve = beat_length - (n/k) x t`. Guessing it is how a four step flow draws its last step eighteen seconds after it was named. Read `out/timing_<episode>.json`, compute each one, re-render. Budget for this pass rather than discovering it.
...
**`panel_stack`****'s pill is a fixed 5.0 units wide**, unlike `panel_columns`, which derives its width from the column count. The "how much fits" reasoning above is built on derived widths and does not apply to it: a stack's layer names have to be short, or the whole group scales down to fit them.
...
- **A still frame** held for more than about six seconds, which means a beat ran out of things to show and left a card on screen while somebody talked over it. That is usually the real defect, and the overlap was only how it announced itself.
  ...
  **The closing beat is where this always bites.** Measured across the first four overviews, every single one ends on a motionless card: 14.9, 18.0, 18.1 and 30.6 seconds of a take sitting still while the narrator delivers the conclusion over it. The pattern is the same each time, because a take draws its claim in one move and then has nothing left to do with half a minute of speech. Give the closing beat something that unfolds with the line: reveal the take in the two or three parts the sentence already has, or bring the map back and mark it as the conclusion lands. Drawing one card and holding it is the default, and the default is the defect.
