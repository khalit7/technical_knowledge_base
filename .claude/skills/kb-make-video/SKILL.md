...
*This copy is the page as Notion last edited it, 2026-09-22 23:52:00 UTC. A procedure*
...
- Spell numbers and names the way they should be said: "three point two two times", "twelve hours", "Z dot A I", "four thousand tokens". Text to speech reads `3.22x` and `Z.ai` badly, so never leave either in a spoken line. **A dotted name orphans if you spell out the dot.** `torch.export` is one token on the panel; saying "torch dot export" gives the word set {torch, dot, export}, which shares nothing with it, and squashes to `torchdotexport`, which does not contain `torchexport`. Say "torch export" and both routes pass. This is a common shape on these pages and the squashed-matching rules do not cover it.
