...
*This copy is the page as Notion last edited it, 2026-09-23 08:57:00 UTC. A procedure*
...
5. **Drop or move a heading,** which costs every item after it one place. On a focus beat it costs more, since even the head is drawn only after the whole lighting time: dropping three heads fixed eight of one episode's nine leads. **A ****`table`****'s heading row is not a heading in this sense**: it carries the column labels and cannot go, so on a table beat go to step 4 instead. Split a row across a question and its answer and give the second a blank first cell; `check_leads` names a blank-label row by its first non-blank cell, and it is timed from every cell.
...
The **page id** is the Notion id of the page the video derives from. `video/tools/episodes.json` already holds every topic overview, with its episode name and its page id, so there is no row to add. A news issue is not in it, so find that one by title through the Notion API and publish it with `upload_all.py --only <episode> --page <page id> --go`. Without `--page` a news episode used to be skipped silently, the run printing `0 uploaded, 0 skipped`; it now refuses an unmapped `--only`. `video/tools/upload_all.py` does a whole batch from that map, and refuses any episode whose script has changed since it was rendered.
...
- **A beat carrying ****`focus`**** delays its own panel** by about **0.25 seconds per parked row**, spent **at the head of the beat**, because `beat()` lights the map before it builds anything. A row is a column heading or a stack layer, and every row is faded, lit or dimmed, so a four column map costs about one second whether you name one label or four. **Until 2026-09-23 it cost a quarter second per registered name**: `compact()` registers every item under its heading's row as well as the heading, and `focus_on` faded once per name, so a four by four map spent five seconds lighting and, because the heading is registered last, focusing on an item's name lit its row and then dimmed it again. `focus_on` now fades each row once, lit if any of its names is wanted. Every episode published before that date carries the old cost in its video, so audit one with `check_leads --per-name-focus`; a new render needs no flag. This figure has been wrong on this page in both directions before, which is what comes of counting the thing that is easy to see instead of reading `compact()` and `focus_on`. That is what decides whether a line pointing at the panel in its opening words is safe. A screen reference in the first line of such a beat lands on an empty frame. The general form of this is worth holding on to, because it bites without `focus` too: reveals are spread across the whole beat, so the first several seconds of a forty five second beat are an empty or half drawn panel whatever the beat carries. A line one second in that asks "who measured these" has nothing to point at. Put the reference after the picture, or hold the picture back with `reserve`.
...
**Re-run every check immediately before publishing, not after the last edit.** `video/tools/prepublish.sh <episode>` runs them all in one go: `review.py` (structure, layout, timing with the longest still frame, voice, delivery), the screen references and `check_leads --words`, each under the GPU lease. In a batch the tools move: one episode had the focus delay start being modelled after its reserves were fitted, worth about five seconds at the head of four of its nine beats. Refitting confirmed them that time and could as easily have invalidated them after the render was paid for.
