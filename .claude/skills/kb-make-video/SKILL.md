...
*This copy is the page as Notion last edited it, 2026-09-22 22:29:00 UTC. A procedure*
...
- **Pace is a writing problem, not a post-processing one.** Slowing the finished audio with a phase vocoder hits the target rate on paper and makes the voice sound processed, which is the exact quality this whole pipeline exists to avoid. It was tried, and rejected on listening. **The best lever is splitting the beat's turns, and it costs nothing.** `render.py` emits one conversation item per turn, so more turns means more prosodic boundaries and a slower read of the identical words. One beat was already 24 sentences averaging under eight words, with no comma left to convert, and fifteen seeds gave a best of 169; redistributing the same words from 7 turns into 11, all still spoken by A, came back at **159**. No words cut and no seconds of narration added, which makes it strictly better than the other two. Reach for it first.
...
**A ****`stack`**** parks too, not just a ****`columns`****.** It collapses to its layer names and registers its focus handles by them, so a stack is a good home frame when the order of the layers is the argument. One catch: `compact()` gives every row of a parked stack the **same tone**, so a stack map cannot colour its stages differently the way a columns map can.
...
What each kind reveals: `stat` 2 (the number, then its caption), `compare` 2 (one per side), `claim` 1 or 2, `columns` one per column, `stack` one per layer, and `points`, `flow`, `bars`, `table` and `resources` one per row or step, plus one for the head where there is one.

**Two heads that behave differently, which is worth holding on to because it moves every landing on the beat.** A `columns` heading is drawn as part of its column and is **not** a reveal of its own, so a three column map is three reveals. A `table`'s head row **is** a reveal of its own, so a table with a head and three rows is four. Count wrong and every reveal after it lands somewhere else. `claim` deserves its own warning: its two reveals mean the `note` lands at `beat_length - reserve`, so **the sentence the note paraphrases has to be the last thing said in the beat.** One draft said it eight seconds in and the card carrying it appeared fifteen seconds later.

So **`stat`****, ****`compare`**** and a two-reveal ****`claim`**** are all roughly 25 second panels**, sharing identical arithmetic. One author built a `stat` beat for a figure, found it could not carry the material, and had to merge it away and rewrite. `compare` is a **short beat's** panel, however well two positions side by side suit the argument: one episode's first draft used it on two long beats and the preview came back with 19.3 and 22.3 second still frames. The fix was `table` and `points`, which carry five and seven reveals. Pick the kind that fits the beat's length, then write the line to it.
...
Two artefacts the rest of this page leans on, since neither says what is in it: `out/timing_<episode>.json` is a **list** of `{key, start, dur, still}` in beat order, and `verification.json` keys each beat to fields named **`said`** and **`heard`**, which is the pair to diff when reading transcripts. Read the file after a targeted fix and you will conclude the fix failed while the renderer is telling you it worked. Re-run the verifier before reading it, every time.
