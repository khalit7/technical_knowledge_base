...
**Distributed training** is a composition problem rather than a single choice: FSDP2 and HSDP for sharded data parallelism, TP inside a node, PP across nodes, EP for MoE experts, CP for the sequence dimension that makes 128k-token training fit. The mechanics, the communication costs and the rules for combining them are on [Distributed Training](distributed-training.md). The HuggingFace **Ultra-Scale Playbook**, 4000+ benchmarked runs distilled into an interactive book, is the canonical modern reference for picking among them.
