...
- **Kaplan et al. 2020**: loss follows power laws in model size N, dataset size D,
  and compute C; concluded you should scale N faster than D. See
  [scaling-laws](../../papers/2020-01_scaling-laws/summary.md).
- **Chinchilla (Hoffmann et al. 2022)**: fixed Kaplan's learning-rate-schedule
  ...
  **D ~ 20 tokens per parameter** at a given compute budget. Chinchilla (70B, 1.4T
  tokens) beat Gopher (280B) at equal compute. See
  [chinchilla](../../papers/2022-03_chinchilla/summary.md).
...
3. **Long-context extension**: short-context pretraining for most tokens, then a
   ...
   (see [positional-encodings.md](positional-encodings.md)).

Data quality work (dedup, filtering, mixing) lives in
`topics/data-curation-and-datasets`. Key idea: data mixture is now the main open
...
- **Semi-open reports** worth reading: [Llama 3](../../papers/2024-07_llama-3/summary.md)
  (data mix, scaling, infra failures), DeepSeek-V3 (fp8 training, MoE at scale,
  see `papers/2024-12_deepseek-v3`), Qwen3 (see `papers/2025-05_qwen3`).
