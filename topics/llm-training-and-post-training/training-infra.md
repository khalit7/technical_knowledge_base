...
- Llama 3 paper, infrastructure section: the best public account of failures at 16k-GPU scale. Summary in [The Llama 3 Herd of Models](../../papers/2024-07_llama-3/summary.md).
...
1. **Hardware**: GPU nodes (8xH100/H200/B200 class) with NVLink/NVSwitch inside
   ...
   FSx, GPFS) or object storage for data and checkpoints. See [Topic: hardware](../hardware/summary.md).
...
5. **Experiment layer**: W&B/MLflow/TensorBoard, config management, data
   versioning. See [Topic: ml-infra-and-orchestration](../ml-infra-and-orchestration/summary.md).

### Training frameworks (2026)

- **Megatron-LM / Megatron-Core**: NVIDIA's TP/PP/EP reference; the kernels and
  parallelism most frontier-scale runs still trace back to [Megatron-LM: Training Multi-Billion Parameter Language Models Using Model Parallelism](../../papers/2019-09_megatron-lm/summary.md).
...
- RL post-training infra (verl, OpenRLHF, SkyRL, NeMo-RL) adds a rollout engine
  (vLLM/SGLang) plus trainer weight-sync; see [Alignment: SFT, RLHF, DPO Family, RLVR](alignment-and-rlhf.md).
...
See also [Distributed Training](distributed-training.md) for the parallelism being orchestrated and [Topic: ml-infra-and-orchestration](../ml-infra-and-orchestration/summary.md) for SLURM/K8s depth.
