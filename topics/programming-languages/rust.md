...
*Created 2026-08-31.* A zero-to-expert path that assumes no C++ background; the C++ mapping is an optional appendix at the end, useful once you have C++ but not required to start.
...
- [**crates.io**](http://crates.io/) is the registry; a package is a **crate**. `cargo add serde` edits `Cargo.toml` for you; `cargo build`, `cargo test`, `cargo run --release`, `cargo doc --open`.
...
- [**docs.rs**](http://docs.rs/) automatically builds and hosts documentation for every published crate, which is why Rust library documentation is uniformly good and uniformly findable.
...
What to build: a real crate with a clean API, documented public items, tests, and a published version on [crates.io](http://crates.io/).
...
- **cargo** is build + deps + test + bench + doc + publish in one: `cargo new`, `cargo add tokio`, `cargo test`, `cargo run --release`. [Crates.io](http://crates.io/) is the registry; `Cargo.lock` behaves like uv.lock.
...
- Also notable: **ort** (ONNX Runtime bindings), tract, [mistral.rs](http://mistral.rs/) (candle-based inference server), lance/lancedb and qdrant (vector storage), tch (libtorch bindings). Training at scale remains PyTorch's; Rust owns data pipelines, tokenizers, serving glue, and edge inference.
- Reading target once you are past Stage 2: candle-transformers' [llama.rs](http://llama.rs/), a readable and complete LLM implementation.
