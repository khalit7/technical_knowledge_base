...
⏱ 22 min read · +40h resources

A zero-to-expert path that assumes no C++ background; the C++ mapping is an optional appendix at the end.
...
- cheats.rs (~45 min): the whole language on one dense page; read it end to end once at Stage 2.
...
- [Rust blog / releases](https://blog.rust-lang.org/) (blog, ~20 min per release post): the release notes, one post per six-week cycle.
...
Rust is the language where this section is shortest, because **cargo** is the build system, test runner, benchmark runner, documentation generator, package manager and publisher in one, and there has never been a competitor.

- **crates.io** is the registry; a package is a **crate**. `cargo add serde` edits `Cargo.toml` for you; `cargo build`, `cargo test`, `cargo run --release`, `cargo doc --open`.
...
- **docs.rs** automatically builds and hosts documentation for every published crate, which is why Rust library documentation is uniformly good and uniformly findable.
...
What to build: a real crate with a clean API, documented public items, tests, and a published version on crates.io.
...
- **cargo** covers build, deps, test, bench, doc and publish in one (see Package management above); `Cargo.lock` behaves like `uv.lock`.
...
- **burn** (Tracel): framework-scale: pluggable backends via its Backend trait (CubeCL, wgpu, tch, candle, ndarray), kernel fusion, autodiff, ONNX import, full training story. More ambitious, heavier abstractions; its GPU-kernel-in-Rust compiler CubeCL targets CUDA, ROCm and wgpu.
- **GPU kernels in Rust**: Nvidia shipped two first-party tracks in September 2026, split the way this subject always splits. **cutile-rs** is the tile track: you write tile-level computation, the compiler decides how tiles map onto each architecture, and the kernel is JIT-compiled through CUDA Tile IR on first use, with exclusive access falling out of tensor partitioning plus ordinary ownership rules. It is published on crates.io and already in production in Hugging Face's Grout inference engine and in `mistral.rs`. **cuda-oxide** is the thread-level track: a custom rustc codegen backend routing `#[kernel]` functions through MIR to PTX, with `DisjointSlice` types and launch contracts turning aliasing between concurrent threads into a compile error. That is the ownership model being asked to carry races between thousands of threads inside one kernel launch, which it has never been asked to do before; the tile track's answer is to change the unit of work so the question does not arise. cuda-oxide is early alpha and needs a pinned nightly toolchain plus LLVM. No performance benchmarks are published and the announcement claims none, so the safety argument is the reason to look rather than speed. CubeCL above is the community attempt at the same thing, now with a first-party neighbour.
...
- The C++ side of the comparison: [C++: zero to expert](cpp.md), [C++: modern practice and standards status](cpp-staying-current.md)
- Building the Python extension: [Python: zero to expert](python.md), [Python: staying current (3.12 to 3.15)](python-staying-current.md)
- Rust in the inference stack: [Topic: inference-and-serving](../inference-and-serving/summary.md)
- Writing the kernels themselves, in either language: [Topic: cuda-and-gpu-programming](../cuda-and-gpu-programming/summary.md)
