...
- **Task YAMLs** declare dataset (HF hub path), `doc_to_text` / `doc_to_target` (Jinja templates or Python callables via `!function`), the `output_type`, metrics, and filters (regex extraction, majority voting for self-consistency). Variants (fewshot counts, chain-of-thought (CoT) versions, translations) are YAML overrides via `include:`. Groups and tags aggregate tasks (e.g. `mmlu` is a group of 57 subtasks).
