...
3. **Post-call (output rails)**: assess the generated output before the user sees it: hallucination/groundedness checks (claim-vs-source for retrieval-augmented generation, RAG), policy violations that only manifest in the output, bias, PII/secret leakage. Most expensive stage (a second model pass over full output) but the only one that can catch generation-side failures. Outcomes: block, regenerate, redact, or annotate.
