...
- **OLTP vs OLAP.** This split, not the vendor, decides row store or column store, and it is why analytics belongs on a replica or in a separate system rather than on your production primary.
...
- **Indexing.** An index is a redundant, ordered copy of a subset of your data that turns a scan into a lookup, and every index makes writes slower and the planner's job harder. Cardinality and selectivity, composite ordering (equality columns then range), covering indexes and sargability are in the deep dive.
...
When a measured limit forces you off the default, five questions decide the replacement, in this order.

1. **What are the access patterns?** Which queries run, how often, against what latency budget. Data shape is the wrong starting point, because nearly any shape fits nearly any model; the queries are what a store is fast or slow at. A key-value store is only cheap if you always know the key, and wide-column is only fast if the partition key matches the query.
...
**DuckDB is the analytics tool to reach for first.** It queries Parquet, CSV, JSON, Iceberg, and Delta in place, over local disk or S3, with full SQL and no server, and it will happily chew through tens of gigabytes on a laptop. For eval result analysis, training data profiling and dedup, filtering a corpus before a run, or debugging why a shard is malformed, `duckdb -c "select ... from 's3://bucket/*.parquet'"` beats a Spark cluster on time to answer. Pandas and Polars both interoperate with it zero-copy through Arrow. Its corporate home changed in August 2026, when AWS bought DuckLabs, the Amsterdam company behind it, while explicitly not acquiring the open-source project: DuckDB stays MIT-licensed under the independent DuckDB Foundation, and Hannes Mühleisen and Mark Raasveldt continue to lead the team and the technical direction. Buying the company and leaving the intellectual property in a foundation is a structure worth recognising when it recurs, and it is why the recommendation here is unaffected.
