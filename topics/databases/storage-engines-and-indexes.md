...
No access method is simultaneously optimal at reading, updating and memory footprint. That is the **RUM conjecture**: improving two of read overhead, update overhead, and memory or space overhead makes the third worse. B-trees buy cheap reads with expensive random updates. LSM-trees buy cheap sequential updates with read amplification and background compaction work. Adding an index buys read speed with space and write cost. Compression buys space with CPU. Every storage decision is a point on that triangle, and the question is never "which is faster" but "which corner am I allowed to give up".

The three amplification factors measure the corners:
...
One Postgres consequence: it uses MVCC with heap tuples, so an UPDATE writes a new tuple version and every index on the table must be updated too, unless the update is HOT (heap-only tuple: no indexed column changed and the new version fits on the same page). Hence wide tables with many indexes update slowly, table bloat is a real operational concern, and autovacuum tuning is a production skill rather than a curiosity.
...
Every durable database fights the same fact: a write is not durable until it reaches stable storage, and that is 10,000 times slower than reaching memory.
...
- **GIN** (generalised inverted index): maps each contained element to the list of rows containing it. This is what makes Postgres good at full-text search, JSONB containment, and array membership. Fast to search, slow to update, which the pending-list mechanism compensates for.
...
- **Bitmap**: one bitmap per distinct value, combined with bitwise AND and OR. Excellent for low-cardinality columns in analytical stores. Postgres does not persist bitmap indexes but builds them on the fly for bitmap heap scans, which is why combining two mediocre indexes can still be fast.
...
Most of those six are estimation failures rather than costing failures, which is what makes a learned planner a plausible attack: train a model on measured outcomes and it corrects the estimator from data instead of from a statistical model of the data. A 4B distillation fine-tuned to write Postgres plans reached a 1.81x geometric mean speedup and a 44.7% latency reduction on the join-heavy queries of the Join Order Benchmark in September 2026, with zero regressions when the best of three candidate plans was taken. The catch is economic rather than technical: generating and measuring three candidates costs far more than the planner does, so it pays only where the same analytic query runs repeatedly, and it makes no sense at all on transactional traffic. The result and its training recipe are on [Topic: databases](summary.md).
...
The useful habit is arithmetic rather than recall: before optimising, multiply the number of operations by the cost of one and check whether the answer is near your measured latency. If it is not, you are optimising the wrong layer, and the gap is usually a round trip you did not count or a cache you assumed was warm.
