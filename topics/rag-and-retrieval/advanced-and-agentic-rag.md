...
- [ReAct: Synergizing Reasoning and Acting in Language Models](../../papers/2022-10_react/summary.md) (45 min): the reason-act loop that agentic retrieval instantiates.
...
- **Advanced RAG**: the naive core wrapped in pre-retrieval (query rewriting,
  ...
  checking) stages; still a fixed directed acyclic graph (DAG). This is [The retrieval pipeline: chunking, hybrid search, query transformation, reranking](retrieval-pipeline.md).

- **Agentic RAG**: retrieval becomes a *tool* inside a ReAct-style (reason, then act) loop. The agent
...
- **Microsoft GraphRAG**: have a large language model (LLM) extract entities and relations, run Leiden community
...
- **LazyGraphRAG**: builds only a cheap concept graph extracted with natural language processing (NLP), not an LLM, at index time
...
- **Latency**: seconds of prefill vs tens of milliseconds of approximate nearest neighbor (ANN) search.
...
- **Freshness and access control lists (ACLs)**: per-query filtering and per-tenant isolation have no
...
- [Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks](../../papers/2020-05_rag/summary.md) (2020): where the term started.
- [Topic: agentic-frameworks](../agentic-frameworks/summary.md): LangGraph and LlamaIndex implementations of these loops; [LangChain and LangGraph](../agentic-frameworks/langchain-and-langgraph.md) covers the LangGraph side in depth.
