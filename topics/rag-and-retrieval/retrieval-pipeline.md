...
- **Late chunking** (Jina): run the *whole document* through a long-context embedding
  ...
  document-wide context (resolved pronouns, section topic) at zero LLM cost. Needs a
...
difficulty (or let an agent decide, see
...
- **Open weights**: Qwen3-Reranker 0.6B/4B/8B and BGE-Reranker-v2.5-Gemma2 rival the
  APIs on BEIR; bge-reranker-v2-m3 is the small self-host workhorse. Self-hosting
...
Attach structured fields (tenant, source, date, ACL, doctype) to every chunk and
filter *during* ANN search, not after. Post-filtering an ANN result list silently
...
- [embeddings-and-vector-search.md](embeddings-and-vector-search.md): the models and
  indexes underneath.
- [rag-evaluation.md](rag-evaluation.md): how to tell whether any of these stages paid
  for itself.
