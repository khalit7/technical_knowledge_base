...
A bare LLM answers from parametric memory with nothing to cite, froze at its training

cutoff, and fills gaps plausibly but wrongly. RAG attacks all three: answers are grounded

in retrieved passages, the knowledge base updates without retraining, and citations fall

out for free. It is also the only
...
- **Embedding + index layer**: three choices, which model, which index, which store. **MTEB (Massive Text Embedding Benchmark)** is the public scoreboard, but treat a high row as a prior rather than a verdict, because models are tuned against it and a legal, code or log corpus reorders the table. **HNSW (Hierarchical Navigable Small World)** is the default **ANN (approximate nearest neighbour)** index, and for the store Postgres with **pgvector** is enough more often than vendors admit, because it keeps chunks, metadata and vectors in one transactional system with real SQL filtering.
...
- **Pipeline layer**: chunking (how a document is cut into retrievable units) moves quality more than embedding-model choice, because a chunk that has lost its document context cannot be matched by a query that assumes that context. The 2026 production default is hybrid search: **BM25 (Best Match 25)** for the exact identifiers, error codes and rare names dense vectors miss, dense vectors for the paraphrase BM25 misses, the two lists fused on ranks by **RRF (Reciprocal Rank Fusion)** because their score scales are incomparable, then a **cross-encoder reranker** over the top 50-150, which buys precision for 30-300 ms and is the highest-leverage single addition to a stack. **Contextual retrieval** (Anthropic), an LLM-written 50-100 token situating prefix prepended to each chunk at index time, is the quality ceiling for static pipelines.
...
- **Evaluation layer**: split the system and measure each half. **recall@k** dominates the retrieval side, because nothing downstream can recover an answer that never entered the prompt, with **nDCG@k (normalised discounted cumulative gain)** the standard for comparing rerankers, whose entire job is moving items up. The generator side is scored by a judge LLM for **faithfulness** (the fraction of the answer's atomic claims entailed by the retrieved context, so low faithfulness means hallucination despite retrieval) and **answer relevance**. Make the numbers trustworthy with a golden set of a few hundred human-labelled queries, and triage recall-first: for every wrong answer, check whether the gold chunk was in the prompt, a single bit that assigns the bug to the retrieval half or the generation half.
