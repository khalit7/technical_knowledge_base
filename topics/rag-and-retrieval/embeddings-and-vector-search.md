...
recall@k yourself (see [rag-evaluation.md](rag-evaluation.md)). Open vs API is mostly a
...
3-8x storage and memory-bandwidth savings, faster ANN search. Common pattern:
...
- **PQ** (product quantization): compress vectors into subvector codebooks; usually
  ...
  anisotropic quantization with SIMD-friendly layouts; state of the art
...
For roughly 70% of production RAG workloads (under ~10M vectors, moderate QPS),
