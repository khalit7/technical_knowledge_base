...
Both give context-independent embeddings; contextual embeddings ([BERT](../../papers/2018-10_bert/summary.md), ELMo) superseded them.
...
The transformer (2017) removed the recurrence entirely and kept only attention: self-attention within encoder and decoder plus cross-attention between them, giving full parallelism over sequence length. See [Attention Is All You Need](../../papers/2017-06_attention-is-all-you-need/summary.md); everything after it lives in topics/llms and topics/llm-training-and-post-training.

Postscript (2024-26): recurrence partially returned as **state-space models** ([Mamba](../../papers/2023-12_mamba/summary.md)) and hybrid SSM-attention stacks, which revive RNN-like $O(1)$-per-token inference with modern training parallelism.

## Cross-links

- Vanishing/exploding gradients in general: [normalisation-and-initialisation.md](normalisation-and-initialisation.md), [debugging-training.md](debugging-training.md)
- Gated activations (GLU family descends from these gates): [activations.md](activations.md)
