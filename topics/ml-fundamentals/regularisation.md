...
For plain SGD the two coincide. For adaptive optimisers (Adam) they do not: L2-in-the-loss gets divided by the per-parameter second-moment term, so weights with large gradients are barely decayed. Decoupling restores uniform decay, which is the whole point of **AdamW** (see [Optimisers and learning-rate schedulers](optimisers-and-schedulers.md)).

### Cross-links

- Overfitting/underfitting decision table: [Debugging training](debugging-training.md)
- Normalisation layers also have a mild regularising effect (batch noise): [Normalisation and initialisation](normalisation-and-initialisation.md)
