...
| MAE | $\|y-\hat y\|$ | Simple, cheap, robust to outliers | Constant gradient magnitude: error of 1 treated the same at scale 1 or 1000, hard to size steps; can jump over the minimum even when error is tiny; non-differentiable at 0 |
| MAPE | $\|y-\hat y\|/\|y\|$ | Scale-normalised, interpretable as a percentage | Undefined when $y=0$; asymmetric: same absolute error gives different loss depending on magnitude of $y$ (pred 70 vs actual 40 -> 0.75; pred 70 vs actual 30 -> 1.33) |
| Huber | quadratic for $\|e\|\le\delta$, linear beyond | Tunable $\delta$; linear tail keeps outliers fairly weighted (not MSE-extreme); curved centre gives well-sized steps near the minimum | Conditionals cost compute at scale; $\delta$ must be tuned; no useful second derivative at the switch point |
...
- **L1 / L2 / elastic net penalties**: additive regularisation terms, not task losses. L1 (lasso) drives weights to exactly zero (sparsity, feature selection); L2 (ridge) shrinks weights toward zero without zeroing them; elastic net combines both. Details in [regularisation.md](regularisation.md).
- **Adversarial loss**: GAN minimax objective; generator and discriminator trained against each other. See the generative-models topic.

## Cross-links

- Class-imbalance handling in practice: [debugging-training.md](debugging-training.md)
- Entropy vs cross entropy vs KL: [metrics.md](metrics.md)
