...
HBM: O(N^2) memory traffic, memory-bound. FlashAttention
([paper](../../papers/2022-05_flashattention/summary.md)) restructures it as a single
fused kernel:
...
4. A real fusion torch.compile does not already do (e.g. RMSNorm + QKV projection
   epilogue, or dequant + GEMV for a quantised format), benchmarked per
   [profiling-and-tools.md](profiling-and-tools.md) methodology.
