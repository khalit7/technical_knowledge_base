...
- **torch.compile first**: it auto-generates Triton for fusions, and its matmul/attention
  ...
  shows a gap. See [../pytorch-ecosystem/summary.md](../pytorch-ecosystem/summary.md).
...
5. Profile Triton kernels exactly like CUDA kernels with ncu
   ([profiling-and-tools.md](profiling-and-tools.md)); `proton` (Triton's bundled
   profiler) is useful for autotune-aware timing.
