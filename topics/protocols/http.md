...
See also: [Real-time and event delivery: WebSockets, SSE, webhooks, long polling](realtime-and-events.md) for SSE and WebSockets, [RPC and API styles: REST, gRPC, GraphQL](rpc-and-apis.md) for gRPC on HTTP/2, [Model Context Protocol (MCP)](mcp.md) for Streamable HTTP.
