...
- **Resources, not functions**: anything nameable gets a URI; you transfer representations of it rather than calling a function.
- **Representations**: the client holds resource state as JSON or HTML; state changes by transferring a new one (PUT replaces the resource).
- **Uniform interface**: a small fixed verb set with universal semantics (GET safe and cacheable, PUT/DELETE idempotent, POST neither) instead of a per-API method vocabulary. The deepest difference from RPC styles.
- **Statelessness**: every request carries all context (auth, cursors), no server-side session, which is what makes horizontal scaling and shared caching work.
- **Cacheability and layered system**: intermediaries cache and route because semantics are visible in the method and headers.
- **HATEOAS** (hypermedia as the engine of application state): responses carry links to the next actions, so clients navigate rather than hardcode URLs. The least-followed constraint; the human web of HTML links and forms is its only mass deployment.

#### REST as practiced

REST as practiced = resource-oriented HTTP + JSON, not Fielding-complete REST; "HTTP API" is the honest term when someone is being pedantic. The Richardson maturity ladder: L0 (one POST endpoint, RPC-in-JSON), L1 (resources with URLs), L2 (HTTP verbs + status codes used correctly: where almost everyone sensibly stops), L3 (HATEOAS hypermedia links: rare outside academia). Aim for honest L2, with a `:verb` custom-action escape hatch for the genuinely non-CRUD (Google AIP-136 style, e.g. `POST /models/m1:deploy`).
...
- **What it is**: end-to-end typesafe RPC for TypeScript monorepos. The server defines procedures (`query` / `mutation` / `subscription`) in plain TS, usually with zod validators; the client imports the router's *type* and gets typed, autocompleted calls. No schema files, no codegen: the TypeScript compiler is the contract.
- **On the wire** it is ordinary HTTP/JSON (batched GETs/POSTs; subscriptions ride SSE or WebSockets), so nothing is protocol-novel: the innovation is entirely in the type-level developer experience.
- **The catch**: the contract exists only at compile time and only in TypeScript, so client and server must live in one codebase and deploy roughly together, and there is no language-neutral artifact another team or language can consume.
