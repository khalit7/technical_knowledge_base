...
Autoregressive generation maps cleanly onto the vocabulary of [RL foundations](foundations.md):
...
with the PPO clipped objective ([Deep RL: from DQN to PPO to MuZero](deep-rl.md)) and the value model with a regression
...
- The baseline is exactly the REINFORCE-with-baseline idea from [Deep RL: from DQN to PPO to MuZero](deep-rl.md); GRPO is
...
imperfect verifiers (extractable answers, weak test suites) reintroduce hacking; see [Reward Hacking](../llm-training-and-post-training/reward-hacking.md). Extending RLVR beyond
