...
- A reward R_t is a scalar feedback signal indicating how well the agent is doing at step t.
- The agent's job is to maximise cumulative reward, not immediate reward.
- RL rests on the **reward hypothesis**: all goals can be described as the maximisation of an
  ...
  breaks in interesting ways for LLMs (see [../llm-training-and-post-training/reward-hacking.md](../llm-training-and-post-training/reward-hacking.md)).
...
Using a model makes a method **model-based**; not using one makes it **model-free**.
...
- Every RL agent must balance the two: pure exploitation gets stuck on the first decent behaviour
  ...
  [model-free-methods.md](model-free-methods.md)), optimistic initialisation, entropy bonuses (deep RL),
...
- You typically need to solve prediction to solve control: you must be able to evaluate policies to
  ...
  iteration (see [dynamic-programming.md](dynamic-programming.md)).
...
- LLM connection: GRPO/PPO rollouts generated by a slightly stale policy make LLM RL mildly
  off-policy; importance ratios are the correction (see [rl-for-llms.md](rl-for-llms.md)).
...
- **Bellman optimality equation** (for the optimal policy):
  ...
  iteration, Q-learning, or SARSA (next two files).
