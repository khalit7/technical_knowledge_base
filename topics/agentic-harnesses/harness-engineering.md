...
*Updated: 2026-09-21 (cross-link list moved to the foot of the page, below the prompt-injection section)*
...
A harness is: a loop, a context policy, a toolset, a permission model, and a verification strategy. This file is the engineering that transfers whether you use Claude Code or build your own on the Agent SDK or the frameworks in [Topic: agentic-frameworks](../agentic-frameworks/summary.md).
...
2. **Egress is the control that actually holds.** Every step of this attack is benign until the final outbound request to an attacker-controlled host. Allowlisting outbound destinations from the tool sandbox, and treating URL construction from conversation state as privileged, stops exfiltration regardless of how the instructions got in. This is the same conclusion the [Personal agents: OpenClaw, Hermes Agent, and how they differ from coding harnesses](personal-agents.md) page reaches from the standing-authority direction.
