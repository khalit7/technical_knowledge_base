...
Anthropic's terminal-first coding agent (launched Feb 2025), now spanning CLI, VS Code and JetBrains extensions, a web UI with cloud sessions, and a GitHub app. The reference implementation of the "harness" idea: a deliberately simple loop surrounded by heavy engineering in context management, permissions and extensibility. Model-locked to Claude in practice, across the Haiku, Sonnet and Opus tiers and the Mythos-class Fable above them, though enterprise routes exist via Bedrock/Vertex.
...
- **Planning as a tool**: the TodoWrite/task list keeps the model honest on multi-step work; Plan Mode (Shift+Tab) is read-only and produces an approved plan before edits.
...
- **Small, sharp toolset**: Read/Write/Edit, Bash (persistent-ish shell), Grep/Glob, WebFetch/WebSearch, Task (subagents), NotebookEdit. Tool descriptions are long and prescriptive: prompt engineering aimed at the tool layer.
...
| --- | --- | --- |
| **`CLAUDE.md`** | Markdown memory auto-loaded into context (enterprise, user `~/.claude/`, project, subdirectory scoped); the cross-tool `AGENTS.md` is a full alternative since 2.1.278 | Always-on facts: build commands, conventions, gotchas. Keep it short; it taxes every request |
...
| **Subagents** | Separate context windows launched via the Task tool, defined in `.claude/agents/*.md` with their own model/tools/prompt | Context isolation (search fan-out returns conclusions, not file dumps) and parallelism |
...
- The **Claude Agent SDK** (TypeScript/Python) is the same loop, tools, permission system, hooks and MCP support as a library: Claude Code minus the TUI, and Anthropic's recommended base for non-coding agents too, and part of [Topic: agentic-frameworks](../agentic-frameworks/summary.md). Detail in [Claude Agent SDK](../agentic-frameworks/claude-agent-sdk.md).
...
### Deployment and organisation controls

Releases 2.1.257 to 2.1.278 (September 2026) changed how the harness is deployed far more than how it feels to use, grouped here by what each changes for a deployer.

- **Repo memory file**: the cross-tool `AGENTS.md` standard is accepted alongside `CLAUDE.md`, in 2.1.277 for projects with no `CLAUDE.md` and in 2.1.278 as a full alternative, which ends the agent-config standardisation argument in favour of the shared standard. `omitClaudeMd` in agent frontmatter stops a subagent loading the memory file at all (2.1.271).
- **Managed MCP servers**: `managedMcpServers` lets an organisation push HTTP and SSE MCP servers to every session rather than relying on per-user config (2.1.259). `CLAUDE_CODE_MCP_STARTUP_WAIT_MS` tunes connection timing (2.1.274), and MCP disconnections raise notifications (2.1.273).
- **Unattended hosts**: `--permission-prompts none` for headless hosts with no human to approve anything (2.1.259); `maxEffortLevel` caps reasoning effort across all providers (2.1.267); per-command `allowed_domains` for Bash, PowerShell and Monitor in auto mode (2.1.271); auto mode defaulting to a **server-side classifier** for Claude API and Enterprise users with no classifier overhead charge, surfaced as an `Auto mode server` row in `/status` (2.1.278).
- **Gateways and proxies**: gateway pricing delivered through managed settings (2.1.268) and a 1x to 10x `multiplier` in model pricing (2.1.271); `--accept-command <sha256>` pins plugin installs (2.1.271); opt-in request header hints via `CLAUDE_CODE_GATEWAY_HINT_HEADERS=1` (2.1.273); `CLAUDE_GATEWAY_PROXY_IS_EGRESS_BOUNDARY=1` plus optional per-upstream `headers:` maps (2.1.278). 2.1.276 fixed **every** request failing with `400 Input tag 'advisor_20260301'` behind an `ANTHROPIC_BASE_URL` proxy, the kind of failure worth knowing the version number of.
- **Policy visibility**: an Organization policy status line (2.1.261).
- **Context budget**: `bashOutputMaxChars` and `taskOutputMaxChars` are configurable up to 128K, the practical cap on tool output entering context in one turn (2.1.261).
- **Extension hygiene**: `/skill-doctor` identifies skills installed but never used (2.1.261); `claude plugin eval` gives reproducible plugin scoring (2.1.269); `/plugin install <plugin> --marketplace <source>` installs from a named source (2.1.275). The middle one matters beyond this page: a vendor shipping harness-extension evaluation as a first-party command is the HarnessDev benchmark question reduced to a CLI flag.
...
3. **Determinism at the edges.** Hooks and the permission system are code, not prompts, so safety and style rules do not depend on model compliance.
4. **Permission model as UX**: default-ask with allowlists (`settings.json` permissions), `--dangerously-skip-permissions` only inside containers; sandboxed bash (v2.0.5+, OS-level filesystem/network isolation) cuts prompt fatigue without widening blast radius.

### Power-user practices

- Curate `CLAUDE.md` ruthlessly; treat it as a prompt you pay for on every request. Use `#` to append memories as you notice recurring corrections.
- Plan Mode first for anything non-trivial; approve the plan, then let it run.
- Subagents for search/review fan-out ("use a subagent to find every caller of X") keep the main context clean; `/compact` at natural checkpoints beats auto-compact firing mid-task.
