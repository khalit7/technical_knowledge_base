...
- The **Claude Agent SDK** (TypeScript/Python) exposes the same loop, tools, permission system, hooks, and MCP support as a library: Claude Code minus the TUI. It is Anthropic's recommended base for building non-coding agents too, and belongs equally to [../agentic-frameworks/](../agentic-frameworks/summary.md).
- GitHub Actions integration and cloud sessions ([claude.ai/code](https://claude.ai/code)) run the harness in managed sandboxes; `claude agents` dashboards track running/blocked sessions.
...
See [harness-engineering.md](harness-engineering.md) for how these ideas generalise across harnesses, and [openai-and-google-harnesses.md](openai-and-google-harnesses.md) for the competing design points.

Added 2026-09-07: five releases shipped between Sep 1 and Sep 5, 2.1.257 to 2.1.261. Two of them change how the harness is deployed rather than how it feels to use. **2.1.259** added `managedMcpServers`, a managed setting through which an organisation pushes HTTP and SSE MCP servers to every session rather than relying on per-user config, and `--permission-prompts none` for headless hosts that must run unattended without a human to approve anything. **2.1.261** added an Organization policy status line, configurable `bashOutputMaxChars` and `taskOutputMaxChars` up to 128K (the practical cap on how much tool output can enter context in one turn), and `/skill-doctor` for identifying skills that are installed but never used. The remaining changes are smaller: 2.1.257 made Claude Fable 5.1 the default model and tightened sandbox safeguards, 2.1.258 fixed macOS 12 launch failures, and 2.1.260 added a fullscreen `/diff` panel, prompt-cache diagnostics and MCP server management inside VS Code. [Changelog](https://code.claude.com/docs/en/changelog) (10 min)
