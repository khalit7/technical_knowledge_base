...
**Parallel fan-out.** The same or different prompts run concurrently and results are aggregated: sectioning (independent subtasks), sampling (N attempts, pick best), map-reduce over documents. A workflow, not really "agents", and often the highest ROI pattern of all; LangGraph's Send API and reducer-merged state, or `asyncio.gather` in a DAG engine, implement it.

**Debate / judge panels.** Multiple agents argue or independently answer; a judge (or vote) selects. Useful for evals (LLM-as-judge ensembles reduce single-judge bias; see [../evaluation-and-llm-judges/](../evaluation-and-llm-judges/summary.md)) and for high-stakes single decisions. Research support is mixed: gains on some reasoning benchmarks, but 2025-2026 work repeatedly finds self-consistency (sample N from one model, majority-vote) matches multi-model debate at equal token budget. Pay for it only when diversity of prompts/models demonstrably beats sampling diversity.
...
- Judge design and failure modes: [../evaluation-and-llm-judges/](../evaluation-and-llm-judges/summary.md)
