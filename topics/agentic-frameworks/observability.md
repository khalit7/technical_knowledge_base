...
- **Braintrust**: evals-first, the primary object being the experiment (dataset x scorer x prompt version), with tracing in service of eval loops. Popular with teams whose bottleneck is "is v3 better than v2", not debugging.
...
Direction of travel: OTel's `gen_ai.*` semantic conventions are becoming the wire format; vendor SDKs increasingly both emit and ingest OTLP, and harnesses now emit natively rather than through a wrapper, with Claude Code shipping OpenTelemetry `effort` attributes and managed-settings events in release 2.1.274 (September 2026). Prefer OTel-compatible instrumentation for anything long-lived; reserve vendor SDKs for their value-add features (evals, prompt management).

The thing to watch on this axis is the hosted agent loop. When the vendor runs the loop, as OpenAI's Agents API and Anthropic's Managed Agents both do since September 2026, the spans between your request and your result are generated inside somebody else's process, and what you can trace is whatever they choose to export. Treat trace fidelity as a procurement question there, not an implementation detail; the harness-side account is in [Topic: agentic-harnesses](../agentic-harnesses/summary.md).
...
- **Cost attribution**: capture token usage per generation, price it (platforms ship model price tables; override for negotiated rates), and roll up by user, feature and session via trace metadata. Doing it at the gateway (LiteLLM virtual keys, budgets per team) and at the trace level, then reconciling, catches both runaway agents and mispriced models.
