...
Last updated: 2026-09-21 (acronyms expanded on first use; Terminal-Bench 4.0, Real-SWE and Phi-Bench added; cross-links to paper pages).
...
exact match. The benchmark that chain-of-thought (CoT) prompting was demonstrated on. Now: >95% for anything
...
within days of the contest, before contamination is possible; also runs the International Mathematical Olympiad (IMO) 2024/25/26
...
- **Verified (500, 2024)**: OpenAI paid engineers to filter out broken tasks
  ...
  2024-25 number. Aug 2026 state of the art (SOTA) ~97% (Claude Opus 5-class); effectively saturated, and
...
never compare across those columns. SWE-bench counts as an agentic benchmark for this reason; see [Agentic benchmarks: GAIA, OSWorld, WebArena, tau2, Terminal-Bench, MCP evals](agentic-benchmarks.md).
...
**Phi-Bench (2026)**: infrastructure engineering rather than application code: 85 open-ended tasks in nine categories (training systems, inference and serving, kernels, hardware adaptation, data infrastructure and others) in three escalating formats, 55 single-file kernel completions, 20 multi-file repository implementations and 10 end-to-end optimisations where the model must find the bottleneck itself. Claude Opus 5 leads at 36.53%, Kimi K3 28.12%, Qwen3.8-Max 27.73%; hardware and edge tasks collapse to 5.4% and longer reasoning budgets do not reliably help. Summary in [Φ-Bench: Can Large Language Models Engineer the Infrastructure That Powers Them?](../../papers/2026-09_phi-bench/summary.md).

### What to actually use in Aug 2026

- Model-level codegen: LiveCodeBench (latest window) + LiveCodeBench Pro rating.
- Agentic engineering: SWE-bench Pro (state the split and scaffold) + Terminal-Bench 4.0 (current since Sep 2026; 2.x scores are not comparable to it) + Real-SWE for a private-code sanity check;
